...
class WriteEncoder():
    ...
    def stitchTo(self, transcode, x:float, y:float):
        self.stepToRange(transcode, x, y, self.maxStitchLength, EmbPattern.STITCH)
        transcode.append([x, y, EmbPattern.STITCH])

    def stepToRange(self, transcode, x:float, y:float, length:float, data:int):
    ...
    def lockStitch(self, transcode, lockposition_x:float, lockposition_y:float, towards_x:float, towards_y:float):
        if distance(lockposition_x, lockposition_y, towards_x, towards_y) > self.maxStitchLength:
            polar = oriented(lockposition_x, lockposition_y, towards_x, towards_y, self.maxStitchLength)
            towards_x = polar[0]
            towards_y = polar[1]
        self.stitchTo(transcode, lockposition_x, lockposition_y)
        self.stitchTo(transcode, towards(lockposition_x, towards_x, .33), towards(lockposition_y, towards_y, .33))
        self.stitchTo(transcode, towards(lockposition_x, towards_x, .66), towards(lockposition_y, towards_y, .66))
        self.stitchTo(transcode, towards(lockposition_x, towards_x, .33), towards(lockposition_y, towards_y, .33))
    ...
    def writeCode(self, pattern_from, pattern_to):
        ...
        while currentIndex < currentIndexEnd:
            ...
            if processingCommand is EmbPattern.STITCH:
                ...
                self.stitchTo(toPoints,current_x, current_y)
                toPoints.append([current_x,current_y, EmbPattern.TRIM])
                toPoints.append([current_x, current_y, EmbPattern.COLOR_CHANGE])
